...
type SymmetricEncrypter interface {
	...
	GenerateSharedSecret(crypto.PublicKey) ([]byte, []byte, error)
}

type secretSealer interface {
	Seal([]byte, []byte, []byte, []byte) ([]byte, error)
	GenerateSharedSecret(delphi.PublicKey) ([]byte, []byte, error)
}

func (msg *Message) Encrypt(recipient delphi.PublicKey, e secretSealer) error {

	if len(msg.Nonce) == 0 {
		msg.Nonce = make([]byte, NonceSize)
		rand.Reader.Read(msg.Nonce)
	}
	if msg.PlainText == nil {
		return errors.New("no plain text to encrypt")
	}

	sec, eph, err := e.GenerateSharedSecret(recipient)
	if err != nil {
