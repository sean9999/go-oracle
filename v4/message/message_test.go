...
func TestMessage_Digest(t *testing.T) {
	...
	t.Run("encrypted message", func(t *testing.T) {
		...
		err := msg.Encrypt(bob.PublicKey(), alice)
		assert.NoError(t, err)
		dig, err := msg.Digest()
		assert.NoError(t, err)
		assert.Equal(t, "0101010101010101010101011209b416531ca683f04c41c5bdd51ff0f2a05774580c6b0cac305ee3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", hex.EncodeToString(dig))
	})
...
func TestMessage_EncryptDecrypt(t *testing.T) {
	...
	t.Run("successful encrypt", func(t *testing.T) {
		err := msg.Encrypt(delphi.PublicKey{}, encrypter)
		require.NoError(t, err)
...
func mustMarshal(v any) []byte {
	...
	case string:
		return []byte(v)
	case smap.LexicalMap[string, string]:
...
func alice(t *testing.T) delphi.KeyPair {
	t.Helper()
	cryptotest.SetGlobalRandom(t, 3)
	return delphi.NewKeyPair()
}

func bob(t *testing.T) delphi.KeyPair {
	t.Helper()
	cryptotest.SetGlobalRandom(t, 4)
	return delphi.NewKeyPair()
}
