...
func TestPrincipal_MustBeValid(t *testing.T) {
	...
	assert.Panics(t, func() {
		malory := NewPrincipal()
		malory.KeyPair[0] = delphi.ZeroKey
		malory.MustBeValid()
	})
	assert.Panics(t, func() {
		malory := NewPrincipal()
		malory.KeyPair[1] = delphi.ZeroKey
		malory.MustBeValid()
	})
}
...
func newPrincipalWithSeed(t *testing.T, seed unit64) *Principal {
	t.Helper()
...
func TestPrincipal_SavePeers(t *testing.T) {
	alice := newPrincipalWithSeed(t, 1)
	bob := newPrincipalWithSeed(t, 3).AsPeer()
	carl := newPrincipalWithSeed(t, 4).AsPeer()
	dan := newPrincipalWithSeed(t, 5).AsPeer()
	alice.AddPeer(bob)
	alice.AddPeer(carl)
	alice.AddPeer(dan)
	assert.True(t, alice.HasPeer(bob.PublicKey))
	someKey := delphi.NewKey()
